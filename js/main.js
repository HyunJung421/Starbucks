...
new Swiper('.notice-line .swiper-container', {
  ...
  autoplay: true,  // 반복 재생 여부
  loop: true  // 무한 재생 여부
});

const currentPage = document.querySelector('.current_page'); // 현재페이지
...
function floatingObject(selector, delay, size) {
  ...
  gsap.to(
    ...
    { // 옵션
      ...
      repeat: -1,  // 무한반복
      yoyo: true,  // 위아래로 움직임
      ease: Power1.easeInOut,  // 움직임 가속도 제어
      delay: random(0, delay)
  });
}
floatingObject('.floating1', 1, 15); // 15는 움직이는 범위
...
spyEls.forEach(function(spyEl) {
  new ScrollMagic
    .Scene({
      ...
      triggerHook: .8, // 보여짐 여부 판단 지점(0 ~ 1 사이 값)
    })
